        ...
        <div className="border-b border-white/10 pb-5 sm:flex sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-2 text-sm text-gray-400">
              Gerencie o acervo de livros que você disponibilizou para a
              comunidade.
            </p>
          </div>
        </div>
        ...
        {isLoading ? (
          <div className="mt-12 flex flex-col items-center justify-center space-y-4">
            ...
            <p className="text-sm text-gray-400">
              Buscando seus livros cadastrados...
            </p>
          </div>
        ) : (
          <div className="mt-8">
            {myBooks.length === 0 ? (
              <div className="text-center rounded-2xl border-2 border-dashed border-white/10 py-12 px-4">
                ...
                <p className="mt-1 text-sm text-gray-400">
                  Você ainda não compartilhou nenhum livro na plataforma.
                </p>
              </div>
            ) : (
              <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                {myBooks.map((book) => (
                  <BookCard
                    ...
                    genre={book.genre}
                    isbn={book.isbn}
                    owner={nomeUsuario || "Você"}
                    userIdDono={book.userId}
                    isOwner={true}
                    onDeleteSuccess={carregarMeusLivros}
                  />
                ))}
              </div>
            )}
