...
export default function BookCard({
  ...
  // ================= FUNÇÃO: SOLICITAR LIVRO =================
  const handleSolicitarLivro = async () => {
    // Verifica se o usuário tem o token antes de tentar solicitar
    const token = localStorage.getItem("token_biblioteca");
    if (!token) {
      mostrarToast(
        "Você precisa estar logado para solicitar um livro.",
        "error",
      );
      return;
    }
    ...
    try {
      ...
      // Envia o bookId para a sua rota POST de criação de requests
      const response = await api.post("/requests", { bookId: id });

      if (response.data && response.data.success) {
        mostrarToast("Livro solicitado com sucesso!", "success");

        // Aguarda o toast sumir e redireciona para a tela de acompanhamento
        setTimeout(() => {
      ...
      const mensagemErro =
        error.response?.data?.error || "Não foi possível solicitar este livro.";
  ...
  const handleAcaoDeletar = async () => {
    ...
    try {
      ...
      mostrarToast("Não foi possível excluir este livro.", "error");
    } finally {
  ...
  const handleSalvarEdicao = async (e: React.FormEvent) => {
    ...
    try {
      ...
      mostrarToast("Não foi possível salvar as alterações.", "error");
    } finally {
      setIsSalvando(false);
    }
  };
      ...
      <div className="group relative overflow-hidden rounded-2xl border border-white/10 bg-gray-800 shadow-lg transition hover:-translate-y-1 hover:border-indigo-500/30">
        ...
        <div className="flex flex-col p-5">
          ...
          <div className="mt-4 border-t border-white/10 pt-4">
            ...
            <p className="mt-1 text-sm font-medium text-gray-200">{owner}</p>
          </div>
          ...
          <div className="mt-5">
            {isOwner ? (
              <div className="flex gap-2">
                ...
                <button
                  type="button"
                  onClick={() => setIsModalDeletarAberto(true)}
      ...
      {isModalAberto && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-2xl border border-white/10 bg-gray-900 p-6 text-white shadow-2xl">
            <div className="mb-6">
              <h3 className="text-xl font-bold">Editar Livro</h3>
      ...
      {isModalDeletarAberto && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm">
          <div className="w-full max-w-sm rounded-2xl border border-white/10 bg-gray-900 p-6 text-center text-white shadow-2xl">
            ...
            <h3 className="mt-4 text-xl font-bold">Remover Livro</h3>
            ...
            <div className="mt-6 flex gap-3">
              <button
                type="button"
                onClick={() => setIsModalDeletarAberto(false)}
                className="flex-1 rounded-xl border border-white/10 bg-gray-800 px-4 py-2.5 text-sm font-semibold text-gray-300 transition hover:bg-gray-700"
              >
                Cancelar
              </button>
